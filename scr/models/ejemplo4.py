...
class ejemplo4:
    ...
    def get_productsDataSource():
        ...
        with connection.cursor() as cursor:
            try:
                query = "SELECT P.NOMBRE as ProductName, M.DESCRIPCION AS Brand, F.DESCRIPCION as Manufacturer," \
                        " FAM.DESCRIPCION AS Family, CAST(ROUND((99-1)* RAND(CHECKSUM(NEWID())+1 ) ,0)AS int) AS Stock" \
                        " FROM Productos P " \
